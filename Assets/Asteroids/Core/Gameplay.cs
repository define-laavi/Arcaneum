...
namespace Arcadeum.Asteroids.Core
{
    ...
    public class Gameplay : MonoBehaviour
    {
        ...
        private static IEnumerator GameOver(Vector2 lastPlayerPosition)
        {
            ...
            const float gameOverSpeed = 0.2f;
            const int ticks = 16;

            _instance._gameOverCanvasGroup.blocksRaycasts = true;
            _instance._gameUICanvasGroup.blocksRaycasts = false;

            for (int i = 0; i < ticks; i++)
            {
                float alpha = (float)(i+1) / ticks;

                _instance._gameOverCanvasGroup.alpha = alpha;
                _instance._gameUICanvasGroup.alpha = 1-alpha;

                yield return new WaitForSecondsRealtime(gameOverSpeed/ticks);
            }
        }
